...
in layout(location=3) mat4 indModelMatrix;
...
void main()
{
	...
	vec4 new_position =  modelTransformMatrix * indModelMatrix * v;
	vec4 projected_position = projectionMatrix * viewMatrix * new_position;
	gl_Position = projected_position;

	vec4 normal_temp = modelTransformMatrix * indModelMatrix * vec4(normal,0);
